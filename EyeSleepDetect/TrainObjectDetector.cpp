...
bool TrainObjectDetector::GenerateObjectDetectorFile()
{
    try
    {
        ...
        if (parser.option("t") || parser.option("cross-validate"))
        {
            ...
            const unsigned long target_size = get_option(parser, "target-size", 100*100);
...
void TrainObjectDetector::pick_best_window_size( const std::vector<std::vector<rectangle> >& boxes,
                                                 unsigned long& width,
...
bool TrainObjectDetector::contains_any_boxes(const std::vector<std::vector<rectangle> >& boxes)
{
    for (unsigned long i = 0; i < boxes.size(); ++i)
    {
        if (boxes[i].size() != 0)
...
void TrainObjectDetector::throw_invalid_box_error_message( const std::string& dataset_filename,
