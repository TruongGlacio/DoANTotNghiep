#include "trainingobjectdetect.h"

TrainingObjectDetect::TrainingObjectDetect(QObject *parent) : QObject(parent)
{
...
void TrainingObjectDetect:: pick_best_window_size (
...
bool TrainingObjectDetect:: contains_any_boxes (
    const std::vector<std::vector<rectangle> >& boxes
)
{
    for (unsigned long i = 0; i < boxes.size(); ++i)
...
void TrainingObjectDetect::throw_invalid_box_error_message (
...
int TrainingObjectDetect::train_object_detector(int argc, char** argv)
{
    try
    {
